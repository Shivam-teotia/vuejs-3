...
describe('UserList', () => {
    it('displays there user in list', async () => {
        render(UserList)
        await waitFor(() => {
            expect(screen.queryAllByText(/user/).length).toBe(3)
        })
    })
    it('dispalys next page button', async () => {
        render(UserList)
        await screen.findByText('user1')
        expect(screen.queryByRole('button', { name: 'Next' })).toBeInTheDocument()
    })
    it('dispalys not previous page button', async () => {
        render(UserList)
        await screen.findByText('user1')
    ...
    describe('when user clicks next', () => {
        it('displays next page', async () => {
            ...
            const firsUserOnPage2 = await screen.findByText('user4')
            expect(firsUserOnPage2).toBeInTheDocument()
        })
        ...
        describe('when user clicks previous', () => {
            it('displays previous page', async () => {
                ...
                const firsUserOnPage1 = await screen.findByText('user1')
                expect(firsUserOnPage1).toBeInTheDocument()
            })
        })
        describe('when last page is loaded', () => {
            it('does no display next page button', async () => {
                const { user } = render(UserList)
                await screen.findByText('user1')
                await user.click(screen.queryByRole('button', { name: 'Next' }))
                await screen.findByText('user4')
